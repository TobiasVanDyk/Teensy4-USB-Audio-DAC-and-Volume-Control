...
The encoder switch switch the display on and off, and also cycle through three different displaymodes, which is changed each time the switch is pressed (i.e. pressed twice). The three displaymodes are Bar graph, and two Binary modes (0 to 11111111) normal and inverted. (It was noted that the mute function is not functional for this Teensy 4 USB Audio device - even using the mute from directly in windows has no effect). It may also be convenient to attach a pushbutton to the Teensy 4 on-off pin - pressing it for 5 seconds will switch the DAC off and pressing it again for 2 seconds will switch it back on.
